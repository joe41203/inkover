...
自動テストで確かめられる範囲と、そうでない範囲がある。**ストア提出前に必ず
両方を通すこと。** v0.1.0 は後者を省いて全く動かないものを公開してしまった。

### 自動（CI で回る）

```bash
pnpm compile && pnpm test && pnpm e2e
```

`pnpm e2e` は拡張を読み込んだ Chrome で描画を検証する（18 項目）。ただし
**content script を手で注入して検証している**ので、アイコンクリックから
注入されるまでの経路は含まれない。

### 手動（ここが自動化できていない）

ヘッドレス環境では MV3 の service worker が起動せず、`action.onClicked` を
発火させられない。そのため以下は人が確かめる:

1. `pnpm build` して `chrome://extensions` から `.output/chrome-mv3` を読み込む
2. 普通の Web ページで**拡張アイコンをクリック** → オーバーレイが出るか
   （v0.1.0 はここで何も起きなかった）
3. 実際に線を描けるか
4. `Cmd/Ctrl+S` で PNG が保存されるか（キャプチャは background 経由なので
   E2E では検証できていない）
5. もう一度アイコンを押して終了できるか
