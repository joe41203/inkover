...
![version](https://img.shields.io/badge/version-0.2.0-fb7185)
...
- ペン・矢印・直線・矩形・楕円・スポットライト・テキスト・ステップ・レーザーの 9 ツール
- 描いた図形はスクロールしてもページに貼り付いて追従する
- 描画込みで画面を PNG 保存できる
- 日本語と英語に対応
...
| `Cmd/Ctrl+S` | 描画込みで PNG 保存 |
| `Alt` + ドラッグ | 直前に描いたものを動かす |
...
オーバーレイはページ全体を覆うので、**iframe（埋め込みの Google Docs や
Notion など）の上にも描けます**。

ただしスクロール追従はページ全体のスクロールにのみ対応しています。iframe や
内部スクロール領域の中身をスクロールしても、描いた図形はその場に留まります。
どの領域に紐付けるべきか一意に決まらないため、あえて追従させていません。
